...
class Renderer(nn.Module):
    def __init__(self, sh_degree=1, white_background=True, radius=1, render="2dgs"):
        super().__init__()
    ...
    def render_empty_img(self, rays, res=512, prex='', device=None):

        if self.render == "2dgs":
            if rays is None:
                return {
                    f"image{prex}": self.bg_color.permute(1,2,0).detach()}
            else:
                return {
                    f"image{prex}": torch.full((res,res,3), self.bg_color[0].item()).to(device).detach(),
                    f"depth{prex}": torch.ones((res,res,1)).to(device).detach(),
                    f"acc_map{prex}": torch.ones((res,res)).to(device).detach(),
                    f"rend_normal{prex}": torch.ones((res,res,3)).to(device).detach(),
                    f"depth_normal{prex}": torch.ones((res,res,3)).to(device).detach(),
                    f"rend_dist{prex}": torch.ones((res,res)).to(device).detach(),
    ...
    def render_img(
        ...
        if self.render == "2dgs":
            ...
            # They will be excluded from value updates used in the splatting criteria.
            return {
                f"image{prex}": rendered_image.permute(1,2,0),
