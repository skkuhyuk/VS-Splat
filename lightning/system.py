...
from lightning.network import GaussianVolumeVoxelPredictor
...
class system(L.LightningModule):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.render = cfg.model.render
        self.loss = Losses(cfg)
        self.net = GaussianVolumeVoxelPredictor(cfg=cfg)

        self.validation_step_outputs = []

    def training_step(self, batch, batch_idx):
        
        out_coarse, out_fine, act = self.net(batch, self.global_step)
        loss, scalar_stats = self.loss(batch, out_coarse, out_fine, act, self.global_step)
        ...
        if 0 == self.trainer.global_step % 100  and (self.trainer.local_rank == 0):
            self.vis_results(out_coarse, out_fine, batch, prex='train', render=self.render)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0 and (self.trainer.local_rank == 0):
            self.vis_results(out_coarse, out_fine, batch, prex='val', render=self.render)
    ...
    def vis_results(self, out_mask, out_rgb, batch, prex, render):
        output_vis = vis_images(out_mask, out_rgb, batch, render)
