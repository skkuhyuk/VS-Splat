...
def vis_appearance(out_mask, out_rgb, batch):
    outputs = {}
    B, V, H, W = batch['tar_rgb'].shape[:-1]

    gt_rgb = batch[f'tar_rgb'].permute(0,2,1,3,4).reshape(B, H, V*W, 3).detach().cpu().numpy() # b n h w 3
    gt_mask = batch[f'mask'].permute(0,2,1,3).reshape(B,H,V*W).detach().cpu().numpy()
    outputs.update({f"gt_rgb":gt_rgb, f"gt_mask":gt_mask})
    if out_rgb:
        pred_rgb = out_rgb[f'image'].detach().cpu().numpy()
        outputs.update({f"pred_rgb":pred_rgb})
    if out_mask:
        if "image_mask" in out_mask:
            pred_mask = out_mask[f'image_mask'].detach().cpu().numpy()
            outputs.update({f"pred_mask":pred_mask})
        if "image_coarse" in out_mask:
            pred_coarse = out_mask[f'image_coarse'].detach().cpu().numpy()
            outputs.update({f"pred_coarse":pred_coarse})
    
    return outputs

def vis_appearance_depth(out_mask, out_rgb, batch):
    outputs = {}
    B, V, H, W = batch['tar_rgb'].shape[:-1]
    ...
    if out_rgb:
        pred_rgb = out_rgb[f'image'].detach().cpu().numpy()
        pred_depth = out_rgb[f'depth'].detach().cpu().numpy()
        ...
        outputs.update({f"pred_rgb":pred_rgb, f"pred_depth":pred_depth_colorlized})

        if 'rend_normal' in out_rgb:
            rend_normal = torch.nn.functional.normalize(out_rgb[f'rend_normal'].detach(),dim=-1)
            rend_normal = rend_normal.cpu().numpy()
            outputs.update({f"rend_normal":(rend_normal+1)/2})
            
            depth_normal = out_rgb[f'depth_normal'].detach().cpu().numpy()
        ...
        if 'img_tri' in out_rgb:
            img_tri = out_rgb['img_tri'].detach().cpu().permute(0,2,3,1).numpy()
            outputs.update({f"img_tri": img_tri})
        if 'feats_tri' in out_rgb:
            feats_tri = out_rgb['feats_tri'].detach().cpu().permute(0,2,3,1).numpy()
            outputs.update({f"feats_tri": feats_tri})

        if 'image_fine' in out_rgb:
            rgb_fine = out_rgb[f'image_fine'].detach().cpu().numpy()
            outputs.update({f"rgb_fine":rgb_fine})
            
            pred_depth_fine = out_rgb[f'depth_fine'].detach().cpu().numpy()
            ...
            if 'rend_normal_fine' in out_rgb:
                rend_normal_fine = torch.nn.functional.normalize(out_rgb[f'rend_normal_fine'].detach(),dim=-1)
                rend_normal_fine = rend_normal_fine.cpu().numpy()
                outputs.update({f"rend_normal_fine":(rend_normal_fine+1)/2})
                
            if 'depth_normal_fine' in out_rgb:
                depth_normal_fine = out_rgb[f'depth_normal_fine'].detach().cpu().numpy()
...
def vis_images(out_mask, out_rgb, batch, render):
    if render == "2dgs":
        return vis_appearance_depth(out_mask, out_rgb, batch)
    elif render == "3dgs":
        return vis_appearance(out_mask, out_rgb, batch)
